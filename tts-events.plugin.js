...
/* global PluginUtilities:true, InternalUtilities:true, BdApi:true */
...
class TTSEvents {
    ...
    getSettingsPanel() {
        ...
        let save_key = this.getShortName() + "-settings"

        let loaded_settings = BDfunctionsDevilBro.loadAllData(this, save_key)
        console.log(A, "LOAD_SETTINGS", loaded_settings, loaded_settings.length)

        if (!jQuery.isEmptyObject(loaded_settings)) {
            this.settings = loaded_settings
        }
        console.log(A, "ACTUAL_SETTINGS", this.settings, "default", this.defaults, "loaded", loaded_settings)

        let html = ''
            + `<div class="bda-header"><span class="bda-header-title"><span class="bda-name">${this.getName() + " Settings"}</span> v<span class="bda-version">${this.getVersion()}</span> by <span class="bda-author">${this.getAuthor()}</span></span></div>`
        for (let key in this.settings) {
            //console.log(A, "iterator", self.settings[key].value, self.defaults[key].value, "input-" + key)
            html += ''
                + `<div class="flex-3B1Tl4 tooltip-custom" style="margin: 5px 0">`
                + `   <h3 style="flex: 0 0 50%; line-height: 38px; align-self:baseline">${this.defaults[key].description}</h3>`
                + `   <div style="flex: 1 1 auto;" class="inputWrapper-3xoRWR vertical-3X17r5 flex-3B1Tl4" style="align-self:baseline">`
                + `     <input type="text" value="${self.settings[key].value}" placeholder="${self.defaults[key].value}" class="inputDefault-Y_U37D input-2YozMi size16-3IvaX_" id="${"input-" + key}">`
                + `     <span class="tooltiptext-custom">${self.defaults[key].tooltip}</span>`
                + `    </div>`
                + `</div>`
        }
...
.tooltip-custom:hover .tooltiptext-custom {
    ...
    load() {
        ...
        if (typeof BDfunctionsDevilBro === "object") {
            ...
            console.error(A, "Could not load BD functions")
        }
    }
